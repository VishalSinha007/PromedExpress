...
public class Checkoutexceptionprocesses_GeneralVerification extends AbstractPage{

	public Checkoutexceptionprocesses_GeneralVerification(WebDriver driver) {
		super(driver);
 ...
 */
	...
	public boolean Order_Confirmation() throws UnsupportedEncodingException, URISyntaxException, IOException {
		...
		Common.waitForElement(driver, By.xpath("//span[text()='YOUR ORDER HAS BEEN PLACED']"));
		Common.pause(2);
	...
	/*
	 * Enter Wrong Credit Card Details
	 */
	
	public boolean CreditcardDetails(){
		
		Common.log("Verifying Wrong Credentials for Credit Card");
		
		WebElement invalidcreditcardmsgdisplay = driver.findElement(By.xpath("//span[contains(.,'Please, enter valid Credit Card Number')]"));
		
		if(invalidcreditcardmsgdisplay.isDisplayed()){
			
			Common.log("Invalid Credit Card Message ====>"+invalidcreditcardmsgdisplay.getText()+"</br>");
			System.err.println("Invalid Credit Card Message ====>"+invalidcreditcardmsgdisplay.getText()+"</br>");
			
			WebElement invalidexpirydatemsgdisplay = driver.findElement(By.xpath("//span[contains(.,'Please, enter valid Expiration Date')]"));
			
			if(invalidexpirydatemsgdisplay.isDisplayed()){
				
				Common.log("Invalid Expiry Date Message ====>"+invalidexpirydatemsgdisplay.getText()+"</br>");
				System.err.println("Invalid Expiry Date Message ====>"+invalidexpirydatemsgdisplay.getText()+"</br>");
		}
		...
		return true;
			...
			Common.log("Error message is not displayed");
